...
def separate_ds(W, I, H, lamb, i_s, A):
    ...
    H = (
        H
        * (W_bar.conjugate().transpose(1, 0) @ I)
    ...
    W_d = (
        W_d_bar
        * (
            Vl @ H_d.conjugate().transpose(1, 0)
...
def process(image_path):
    img = cv2.imread(os.path.join(folder, image_path), cv2.IMREAD_COLOR)
    n_row, n_col, n_ch = img.shape
    M = 3
    N = n_row * n_col
    R = 7
    img = img.reshape(N, M).conjugate().transpose(1, 0)
    ...
    max_iter = 10000
    while True:
        W, H, F_t = separate_ds(W, img, H, lamb, i_s, A)
    ...
    img = img.conjugate().transpose(1, 0).reshape(n_row, n_col, n_ch)
