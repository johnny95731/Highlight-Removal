...
def process(image_path):
    ...
    ind_cmb = np.where(
        (Itemp[:, 0] > th) & (Itemp[:, 1] > th) & (Itemp[:, 2] > th)
    )[0]
    Imask_cmb[ind_cmb] = 1
    ...
    Imask_all = np.zeros(
        height * width
    )  # Pixels with combined reflection or diffuse reflection
    Imask_processed = -1 * np.ones(height * width)  # Processed pixel
    ind_all = np.where((Imask_cmb == 1) | (Imask_df == 1))[0]
    Imask_all[ind_all] = 1
    Imask_processed[ind_all] = (
        0  # -1: not considered; 0: not processed; 1: processed
    )
    ...
    while True:
        ...
        if len(ind_0) > 0:  # If there are un-processed diffuse pixels
            ...
            Y = np.max(Imax_sub)
            ind = np.where(
                (Imax == Y) & (Imask_processed == 0) & (Imask_df == 1)
            )[0]
            ind = ind[0]
            ...
            ind = np.where(
                (c_diff_sum < threshold_chroma) & (Imask_processed == 0)
            )[0]
            ...
            if ind > 0:
                ...
                ind = np.where(
                    (Imask_processed == 0)
                    & (c_diff_sum < Y + 0.1 * threshold_chroma)
                )[0]
                ...
                if len(ind) > 0:
                    ...
                    ind = np.where(Imask_processed == 1)[0]
                    # print(f'{len(ind)} / {len(ind_all)}')
    Image.fromarray(np.uint8(Idf.reshape(height, width, 3))).save(
        os.path.join('result', image_path)
    )
    # Display images
